# ===============================================
# GemmaCLI Tool - generate_terminal_art.ps1
# Responsibility: Renders a 16x16 character art canvas using sparse coordinate updates.
# ===============================================

function Invoke-GenerateTerminalArt {
    param(
        [string[]]$updates,
        [string]$title = "Terminal Composition"
    )
...
$ToolMeta = @{
    ...
    Example          = "<tool_call>{ `"name`": `"generate_terminal_art`", `"parameters`": { `"updates`": [`"8,8,Red,Solid`", `"9,8,Red,Solid`", `"8,9,White,Medium`"], `"title`": `"Target Insight`" } }</tool_call>"
    FormatLabel      = { param($p) "$($p.title)" }
    Execute          = { param($params) Invoke-GenerateTerminalArt @params }
    Behavior         = "The logical canvas is 16x16. Each logical pixel is rendered as a pair of characters (Width x 2). (1,1) is Top-Left, (16,1) is Top-Right, (1,16) is Bottom-Left, (16,16) is Bottom-Right. Assume the canvas is initially all Black Solid blocks. Only provide 'updates' for pixels that should be a different color or shade. Use 'solid', 'dense', 'medium', or 'light' for shading. Colors: Black, DarkBlue, DarkGreen, DarkCyan, DarkRed, DarkMagenta, DarkYellow, Gray, DarkGray, Blue, Green, Cyan, Red, Magenta, Yellow, White."
     ToolUseGuidanceMajor = @"
        - Use this tool to draw 16x16 terminal art.
        - The available colors are: Black, DarkBlue, DarkGreen, DarkCyan, DarkRed, DarkMagenta, DarkYellow, Gray, DarkGray, Blue, Green, Cyan, Red, Magenta, Yellow, and White.
"@

    ToolUseGuidanceMinor = @"
        - Use this tool to draw 16x16 terminal art. Use colors White or Red. 
"@
}
