...
$ToolMeta = @{
    ...
    ToolUseGuidanceMajor = @"
        - When to use 'lookup': Use this tool for quick factual lookups of time-sensitive information that may have changed since your training data cutoff. This includes current software versions, political leaders, CEO/leadership, etc.
        - Important parameters for 'lookup': 
            - `query`: Formulate a concise and direct factual question (e.g., "latest Python version", "current Prime Minister of UK").
        - What NOT to use for: This tool is NOT suitable for recent news, sports scores, stock prices, or niche topics where `brave_search` might be more appropriate.
        - Output Interpretation: Pay attention to the "DIRECT ANSWER", "SUMMARY", and "CURRENT FACTS" sections. If the tool returns "No current data found", inform the user and rely on your training knowledge.
"@
    ToolUseGuidanceMinor = @"
        - Purpose: Find current facts (e.g., software versions, leaders).
        - Basic use: Ask a direct `query` question (e.g., "Python version").
        - Important: Not for news or complex topics.
"@
}